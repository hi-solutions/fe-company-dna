...
export function Dialog({ open, onOpenChange, children }: DialogProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            />
            <div className="z-50 grid w-full max-w-lg gap-4 bg-background p-6 shadow-lg duration-200 sm:rounded-lg animate-in fade-in-90 zoom-in-95">
                {children}
            </div>
        </div>
    )
...
export function DialogContent({ children, className }: React.HTMLAttributes<HTMLDivElement>) {
    return <div className={cn("relative flex flex-col gap-4", className)}>{children}</div>
}
